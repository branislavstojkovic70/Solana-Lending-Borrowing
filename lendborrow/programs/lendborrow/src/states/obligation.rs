...
impl Obligation {
    ...
    pub fn withdraw(&mut self, collateral_index: usize, withdraw_amount: u64) -> Result<()> {
        ...
        // ✅ CHANGED: Računam koliko se smanjuje market value
        let withdraw_value = collateral.market_value
            .checked_mul(withdraw_pct)
            .and_then(|v| v.checked_div(1_000_000_000_000_000_000))
            .ok_or(crate::errors::LendingError::MathOverflow)?;
        
        // ✅ CHANGED: Ažuriram obligation's total deposited value
        self.deposited_value = self.deposited_value
        ...
        if withdraw_amount == collateral.deposited_amount {
            ...
            // ✅ CHANGED: Ažuriram i deposited_amount i market_value
            collateral.withdraw(withdraw_amount)?;
            collateral.market_value = collateral.market_value
                .checked_sub(withdraw_value)
                .ok_or(crate::errors::LendingError::MathOverflow)?;
    ...
    // ✅ NEW: Dodao sam funkciju za proveru health-a
    ...
    pub fn add_collateral(&mut self, collateral: ObligationCollateral) -> Result<()> {
        ...
        self.data_flat.splice(insert_position..insert_position, collateral_bytes);
    ...
    pub fn find_collateral(&self, deposit_reserve: Pubkey) -> Result<(ObligationCollateral, usize)> {
        let mut offset = 0;
    ...
    fn remove_collateral(&mut self, index: usize) -> Result<()> {
        require!(
            index < self.deposits_len as usize,
            crate::errors::LendingError::InvalidObligationIndex
        );
        ...
        let offset = index * ObligationCollateral::LEN;
        self.data_flat.drain(offset..offset + ObligationCollateral::LEN);
        self.deposits_len = self.deposits_len.checked_sub(1).unwrap();
        Ok(())
    }
    
    fn remove_liquidity(&mut self, index: usize) -> Result<()> {
        require!(
            index < self.borrows_len as usize,
            crate::errors::LendingError::InvalidObligationIndex
        );
        ...
        let offset = (self.deposits_len as usize * ObligationCollateral::LEN) 
            + (index * ObligationLiquidity::LEN);
        self.data_flat.drain(offset..offset + ObligationLiquidity::LEN);
        self.borrows_len = self.borrows_len.checked_sub(1).unwrap();
        Ok(())
    }
}

// ✅ CHANGED: Dodao sam InitSpace derive
